...
init(Host, PumpTyp_Pid, PipeInst_Pid, RealWorldCmdFn) -> 
	...
	%%% Added to retrieve pump flow from a pipe instance ! %%%
	% Pump knows the resource instance pipe, but the pipe does not know the pump and flow meter only loops over pipes !
	{ok, [Location]} = resource_instance:list_locations(PipeInst_Pid),
	location:arrival(Location, {pump, self()}),
	%%%
