...
influence({C, _ , Iter }, Acc) ->
	...
	if
		is_tuple(Visitor) == true ->
			{Type, ResourceInst_PID} = Visitor,	
			if 
				Type == pump ->	{ok, InflFn} = pumpInst:flow_influence(ResourceInst_PID); % Pump has a different flow influence function
				true -> {ok, InflFn} = apply(resource_instance, get_flow_influence, [C])
			end;
		true ->
